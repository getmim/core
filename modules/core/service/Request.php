...
class Request extends \Mim\Service
{
    ...
    public function __construct(
        string $file_gates = '/etc/cache/gates.php',
        string $file_routes = '/etc/cache/routes.php'
    ) {
    ...
    {
        $user_aget = $this->getServer('HTTP_USER_AGENT');
        ...
        $content_type = $this->getServer('CONTENT_TYPE');
        ...
        if ($this->isCLI()) {
            ...
            $paths = trim('/' . trim(preg_replace('!\?.*$!', '', $uri), '/'));
            $this->_props['path'] = $paths;
        }
    }
    ...
    public function getCondDateRange(
        string $ds = 'ds',
        string $de = 'ds',
        bool $time = true
    ): ?array {
        ...
        if ($cols['ds'] && $cols['de']) {
            return ['__between', $cols['ds'], $cols['de']];
        }
